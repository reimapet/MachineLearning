...
Repeat same data cleaning process to *train_test* dataset.
...
As we are using tree models we don't have to remove correlating parameters.
...
```
Model accuracy (out of sample) is still 98,8% percent for model with less parameters with the training test set, so the model seems to be good. 
...
answers == answers2
```
From the output we can conclude both models perform identically with the test dataset. Actual results are hidden and just result comparison is shown. To avoid overfitting at this stage it would seem likely the model with only top 20 parameters should be chosen as the final model.

# Conclusions
We were able to achieve 98.8 accuracy with a random forest model and were able to predict all the test cases perfectly.
